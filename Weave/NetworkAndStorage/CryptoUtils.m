...
+ (void) createManager
{
  ...
  if ([weaveService canConnectToInternet])
  {
    ...
    {   
      [weaveService performSelectorOnMainThread:@selector(login) withObject:nil waitUntilDone:NO];
    } 
  }
  else //no network
  {
...
- (id) init
{
	if (self = [super init]) 
	{
		...
		else if (_storageVersion > 5)
		{
			...
			NSException *e = [NSException exceptionWithName:@"Initialization Failed"
				reason: NSLocalizedString(@"Data on the server is in a new and unrecognized format. Please update Foxbrowser.", @"update Foxbrowser") userInfo:nil];
			@throw e;
		}
...
- (NSString *)UUID {
    ...
    NSString *file = [path stringByAppendingPathComponent:@"uuid"];
    NSError *error;
    NSString *uuidString = [NSString stringWithContentsOfFile:file encoding:NSUTF8StringEncoding error:&error];
    if (error) {
        CFUUIDRef uuid = CFUUIDCreate(kCFAllocatorDefault);
        CFStringRef uuidStringRef = CFUUIDCreateString(kCFAllocatorDefault, uuid);
        uuidString = [((NSString *)uuidStringRef) stringByReplacingOccurrencesOfString:@"-" withString:@"0"];
        [uuidString writeToFile:file atomically:NO encoding:NSUTF8StringEncoding error:NULL];
        CFRelease(uuid);
 ...
 //                 keyAppLabel,                        kSecAttrApplicationLabel,  //this was being used incorrectly, and is unneeded, since we lookup by name
	...
	if (err != noErr) 
  {
    NSString* name = [[NSString alloc] initWithData:keyNameUTF8 encoding:NSUTF8StringEncoding];
		NSLog(@"failed to add key \"%@\"   error: %ld", name, err);
    [name release];
	}
...
- (NSData *) decryptSymmetricKey:(NSData *)symKey withPrivateKey:(SecKeyRef) privateKey
{
	...
	if (err != noErr) 
  {
		NSLog(@"Failed to decrypt symmetric key, error: %ld", err);
